...
import { supabase, ensureSupabaseBucket, checkSupabaseStorage } from '../utils/supabaseClient';
...
const runStorageTests = async (): Promise<void> => {
  ...
  try {
    ...
    if (bucketResult.ok) {
      ...
      console.error(`❌ Failed to create bucket "${bucketName}":`, bucketResult.error);
    }
