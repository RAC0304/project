...
export const testReviewsData = async () => {
  ...
  try {
    ...
    console.log('Tour guide IDs in reviews:', [...new Set(tourGuideIds?.map(r => r.tour_guide_id))]);
...
export const createTestReviews = async () => {
  try {
    ...
    const { data: tourGuides } = await supabase.from('tour_guides').select('id, user_id').limit(5);
    ...
    const testReviews = [
      {
        ...
        content: 'The tour guide was very knowledgeable and friendly. Had a great time exploring the destination.',
        is_verified: true,
        helpful_count: 3
      },
      {
        ...
        content: 'Really enjoyed the local insights and recommendations. Would definitely book again.',
        is_verified: true,
        helpful_count: 1
      },
      {
        ...
        content: 'Professional service and very accommodating to our needs. Highly recommended!',
        is_verified: true,
        helpful_count: 2
      }
    ];
