...
import { calculateGuideStats, getUpcomingTours, recentReviews, tours as initialTours } from '../data/tourGuideDashboardData';

const TourGuideDashboard: React.FC = () => {
  const [activePage, setActivePage] = useState('dashboard');
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [tourToEdit, setTourToEdit] = useState<TourData | null>(null);
  const [tours, setTours] = useState<TourData[]>(initialTours); const [toast, setToast] = useState<{
    isVisible: boolean;
    type: ToastType;
    message: string;
  }>({
    ...
    try {
      ...
      if (isUpdating) {
        ...
        setTours(prev => prev.map(tour =>
          tour.id === tourData.id ? tourData : tour
        ));
  ...
  const handleDeleteTour = async (tourId: number) => {
    try {
      ...
      showToast('success', `Tour "${tourToDelete?.title}" deleted successfully!`);
    } catch (error) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      />
      <main className="flex-1 overflow-auto lg:pl-80 pt-4 px-4 transition-all duration-300">
        ...
        {activePage === 'profile' && <ProfileContent user={user} />}        {activePage === 'tours' && <ToursContent
          tours={tours}
