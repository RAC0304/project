...
const AnalyticsContent: React.FC<AnalyticsContentProps> = ({ user }) => {
    ...
    const [overview, setOverview] = useState(getAnalyticsOverview());

    // State for reports
    const [reports, setReports] = useState<AnalyticsReport[]>(getAllReports());
    const [filteredReports, setFilteredReports] = useState<AnalyticsReport[]>(reports);
    const [searchTerm, setSearchTerm] = useState('');
    const [filterType, setFilterType] = useState<AnalyticsReportType | 'all'>('all');

    // State for top data
    const [topDestinations, setTopDestinations] = useState<TopDestinationItem[]>(getTopDestinations());
    const [topTourGuides, setTopTourGuides] = useState<TopTourGuideItem[]>(getTopTourGuides());
    ...
    return (
        <>
            {/* Notification */}
            {notification.show && (
                <div className={`fixed top-4 right-4 z-50 p-4 rounded-md shadow-md ${notification.type === 'success' ? 'bg-green-100 border border-green-200 text-green-800' : 'bg-red-100 border border-red-200 text-red-800'
                    }`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {topDestinations.map((destination, index) => (
                                    <tr key={index}>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{destination.name}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{destination.bookings}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">${destination.revenue.toLocaleString()}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 flex items-center">
                                            {destination.rating}
                                            <Star className="w-4 h-4 text-yellow-500 ml-1 inline" fill="currentColor" />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                ...
                {/* Top Tour Guides */}
                <div className="bg-white rounded-lg shadow-md p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Top Tour Guides</h3>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {topTourGuides.map((guide, index) => (
                                    <tr key={index}>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{guide.name}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{guide.bookings}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">${guide.revenue.toLocaleString()}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 flex items-center">
                                            {guide.rating}
                                            <Star className="w-4 h-4 text-yellow-500 ml-1 inline" fill="currentColor" />
                                        </td>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-6">
                    ...
                    <div className="flex items-center">
                        ...
                        <select
                            ...
                            value={filterType}
                            onChange={(e) => setFilterType(e.target.value as AnalyticsReportType | 'all')}
                        >
                ...
                />
            </div>            {/* Trends Graph Section */}
            <div className="bg-white rounded-lg shadow-md p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Key Trends</h3>
                <div className="text-sm text-gray-500 mb-6">
                    Six-month trend analysis of key metrics. View detailed breakdowns in the reports section.
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="bg-gray-50 p-4 rounded-lg">
                        <AnalyticsChart
                            data={getTimeSeriesData('bookings')}
                            title="Monthly Bookings"
                            type="bar"
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        <AnalyticsChart
                            data={getTimeSeriesData('revenue')}
                            title="Monthly Revenue"
                            type="area"
            ...
            </div>            {/* Modal for creating/editing reports - Using our ReportFormModal component */}
            <ReportFormModal
                isOpen={isModalOpen}
