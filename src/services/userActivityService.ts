...
class UserActivityService {
     ...
     */
    async getUserRecentActivities(userId: string | number, limit: number = 3): Promise<ActivityItem[]> {
        try {
            ...
            const requests = await this.getRecentItineraryRequests(userIdNumber, fetchLimit);
     ...
     */
    private async getRecentBookings(userId: number, limit: number): Promise<ActivityItem[]> {
        try {
        ...
        `)
            ...
            return (data || []).map(booking => {
                ...
                return {
                    ...
                    description: `Booked "${tour?.title || 'Unknown Tour'}" in ${tour?.location || 'Unknown Location'} for ${booking.participants} participants`,
     ...
     */
    private async getRecentMessages(userId: number, limit: number): Promise<ActivityItem[]> {
        try {
            const { data, error } = await supabase
                .from('messages')
                .select(`
        ...
        `)
            ...
            return (data || []).map(message => {
                ...
                const shortContent = message.content.length > 50
     ...
     */
    private async getRecentItineraryRequests(userId: number, limit: number): Promise<ActivityItem[]> {
        try {
            const { data, error } = await supabase
        ...
        `)
            ...
            return (data || []).map(request => {
                const itinerary = Array.isArray(request.itineraries) ? request.itineraries[0] : request.itineraries;
                return {
                    id: `request-${request.id}`,
                    type: 'tour_request' as const,
                    title: 'Tour Request Submitted',
                    description: `Requested "${itinerary?.title || 'Unknown Itinerary'}" for ${request.group_size} people from ${formatIndonesianDate(request.start_date)} to ${formatIndonesianDate(request.end_date)}`,
     ...
     */
    private async getAllItineraryRequests(userId: number): Promise<ActivityItem[]> {
        try {
            const { data, error } = await supabase
                .from('itinerary_requests')
                .select(`
    ...
    async getActivitySummary(userId: string | number): Promise<{
        ...
        try {
            ...
            const [bookingsResult, messagesResult, requestsResult] = await Promise.all([
                supabase.from('bookings').select('id', { count: 'exact', head: true }).eq('user_id', userIdNumber),
                supabase.from('messages').select('id', { count: 'exact', head: true }).eq('sender_id', userIdNumber),
                supabase.from('itinerary_requests').select('id', { count: 'exact', head: true }).eq('user_id', userIdNumber)
            ]);
