 ...
 */
export const getDestinations = async (filters: DestinationFilters = {}): Promise<DestinationResponse> => {
    try {
      ...
      `)
        ...
        const destinations: Destination[] = data.map((item: any) => ({
            ...
            images: item.destination_images?.map((img: any) => img.image_url) || [item.image_url].filter(Boolean),
            attractions: item.attractions?.map((attraction: any) => ({
            ...
            category: item.destination_categories?.map((cat: any) => cat.category) || [],
    ...
    try {
      ...
      `)
        ...
        const destination: Destination = {
            ...
            shortDescription: data.short_description || data.description.substring(0, 150) + '...',
            imageUrl: data.image_url || (data.destination_images?.[0]?.image_url) || '',
            images: data.destination_images?.map((img: any) => img.image_url) || [data.image_url].filter(Boolean),
            attractions: data.attractions?.map((attraction: any) => ({
                id: attraction.id.toString(),
                name: attraction.name,
                description: attraction.description,
                imageUrl: attraction.image_url || ''
            ...
            category: data.destination_categories?.map((cat: any) => cat.category) || [],
 ...
 */
export const getDestinationCategories = async (): Promise<DestinationCategory[]> => {
    try {
        ...
        return ['beach', 'mountain', 'cultural', 'adventure', 'historical', 'nature', 'city']; // fallback
    }
};
 ...
 */
export const searchDestinations = async (searchTerm: string, limit: number = 20): Promise<Destination[]> => {
    try {
        if (!searchTerm.trim()) {
            return [];
        }
 ...
 */
export const getDestinationsByCategory = async (categories: DestinationCategory[], limit: number = 20): Promise<Destination[]> => {
    try {
        const result = await getDestinations({
            categories,
            limit
