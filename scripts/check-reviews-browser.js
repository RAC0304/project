...
async function checkReviewsData() {
  try {
    ...
    const { debugTourGuideReviews } = await import('./src/services/reviewService');
    const { getTourGuideIdByUserId } = await import('./src/services/tourGuideService');
    ...
    if (debugResult && debugResult.reviewsForTourGuide > 0) {
      console.log('✅ Found', debugResult.reviewsForTourGuide, 'reviews for tour guide ID', tourGuideId);
    } else {
