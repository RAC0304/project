...
const supabaseKey =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxiaHFwamd6cW1rcW1yd3libHhkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzMwNDAwMzgsImV4cCI6MjA0ODYxNjAzOH0.m2c_JOaNDbBpfklLI6VjqsZl_4Qsv2JsW1R8cXY2-XE";
...
async function testTourGuideMapping() {
  ...
  for (const user of tourGuideUsers || []) {
    ...
    if (tourGuideError) {
      console.error(
        `Error fetching tour_guides for user ${user.id}:`,
        tourGuideError
      );
      continue;
    }
    ...
    if (!tourGuideRecords || tourGuideRecords.length === 0) {
      ...
      for (const tg of tourGuideRecords) {
        ...
        if (reviewsError) {
          console.error(
            `Error fetching reviews for tour_guide ${tg.id}:`,
            reviewsError
          );
        } else {
          console.log(
            `  📝 Reviews for tour_guide ${tg.id}:`,
            reviews?.length || 0,
            "reviews"
          );
