...
async function testProfilePictureUpload() {
  ...
  console.log(`Base64 data size: ${sizeInBytes} bytes (${sizeInMB.toFixed(4)} MB)`);
  console.log(`✅ Size check: ${sizeInMB < 5 ? 'PASS' : 'FAIL'} (under 5MB limit)`);
  ...
  console.log(`Valid URL length: ${validUrl.length} chars - ${validUrl.length < 2000 ? 'PASS' : 'FAIL'}`);
  console.log(`Long URL length: ${longUrl.length} chars - ${longUrl.length < 2000 ? 'PASS' : 'FAIL'}`);
  ...
  Object.entries(testFields).forEach(([field, value]) => {
    ...
    console.log(`${field}: ${value.length} chars - ${isValid ? 'PASS' : 'FAIL'}`);
  });
  ...
  console.log('ALTER TABLE public.users ALTER COLUMN profile_picture TYPE TEXT;');
}
